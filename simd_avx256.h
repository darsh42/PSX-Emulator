...
static inline void simd_set_triangle_data(int32_t x0, int32_t y0, uint32_t c0,
    ...
    triangle = (struct triangle) {
        ...
        .xm = {
            _mm256_set1_ps(y1 - y2),
            _mm256_set1_ps(y2 - y0),
            _mm256_set1_ps(y0 - y1),
        },
        .ym = {
            _mm256_set1_ps(x2 - x1),
            _mm256_set1_ps(x0 - x2),
            _mm256_set1_ps(x1 - x0),
        },
        .add = {
            _mm256_set1_ps(y2*x1 - y1*x2),
            _mm256_set1_ps(y0*x2 - y2*x0),
            _mm256_set1_ps(y1*x0 - y0*x1),
        },
        .recp_area =
...
static inline vectorf simd_compute_equation(vectorf x, vectorf y, enum equation_component c) {
    ...
    return _mm256_mul_ps(triangle.recp_area,
                         _mm256_add_ps(triangle.add[c], 
                         _mm256_add_ps(_mm256_mul_ps(triangle.xm[c], x),
                                       _mm256_mul_ps(triangle.ym[c], y))));
}
...
static inline void simd_compute_colors(uint32_t *colors, vectorf alphas, vectorf betas, vectorf gammas, vectorf min, vectorf max) {
    ...
    vectorf   redf = 
        _mm256_add_ps(_mm256_mul_ps(triangle.r[0], alphas), 
                      _mm256_add_ps(_mm256_mul_ps(triangle.r[1], betas), 
                                    _mm256_mul_ps(triangle.r[2], gammas)));
    vectorf greenf = 
        _mm256_add_ps(_mm256_mul_ps(triangle.g[0], alphas), 
                      _mm256_add_ps(_mm256_mul_ps(triangle.g[1], betas), 
                                    _mm256_mul_ps(triangle.g[2], gammas)));
    vectorf  bluef = 
        _mm256_add_ps(_mm256_mul_ps(triangle.b[0], alphas), 
                      _mm256_add_ps(_mm256_mul_ps(triangle.b[1], betas), 
                                    _mm256_mul_ps(triangle.b[2], gammas)));
